...
export default function InsurancePrivate({ insurancePrivate, onUpdate }: InsurancePrivateProps) {
  ...
  const [editedRows, setEditedRows] = useState<InsurancePrivateType[]>([])
  ...
  function startEditMode() {
    setEditedRows(insurancePrivate.map((i) => ({ ...i })))
    setEditMode(true)
  }
  ...
  function updateCell(id: string, field: keyof InsurancePrivateType, value: string) {
  ...
  const rows = editMode ? editedRows : insurancePrivate
                ...
                return (
                  <tr key={ins.id}>
                    ...
                    <td className={`${tdClass} font-semibold`}>
                      {editMode ? (
                        <input className={cellInputClass} value={ins.companyName} onChange={(e) => updateCell(ins.id, 'companyName', e.target.value)} />
                      ) : ins.companyName}
                    </td>
                    ...
                    <td className={tdClass}>
                      {editMode ? (
                        <input className={cellInputClass} value={ins.policyName || ''} onChange={(e) => updateCell(ins.id, 'policyName', e.target.value)} />
                      ) : ins.policyName || '—'}
                    </td>
                    <td className={tdClass}>
                      {editMode ? (
